...
<body class="bg-gray-50 antialiased">
    ...
    if (isset($_SESSION['errors']) && !empty($_SESSION['errors'])):
        $errors = $_SESSION['errors'];
    ?>
        <div id="toast-default" class="fixed top-4 right-4 flex items-center w-full max-w-xs p-4 text-body bg-red-100 text-red-800 rounded-lg shadow-lg border border-red-300 z-50" role="alert">
            <svg class="w-6 h-6 text-yellow-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.29 3.86 1.82 18a1 1 0 0 0 .86 1.5h18.64a1 1 0 0 0 .86-1.5L13.71 3.86a1 1 0 0 0-1.42 0zM12 9v4M12 17h.01" />
            </svg>
            <div class="ms-2.5 text-sm ps-3.5 flex-1">
                <?php echo implode('<br>', $errors); ?>
            </div>
            <button type="button" onclick="this.parentElement.remove()" class="ms-auto flex items-center justify-center text-red-800 hover:text-red-900 bg-transparent border border-transparent rounded text-sm h-8 w-8 focus:outline-none">
    ...
    <?php
        // Clear session errors after showing
